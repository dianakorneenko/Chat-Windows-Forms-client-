...
namespace HelloApp
{
    public class Chat : Form
    {
        ...
        public Chat(NetworkStream stream, TcpClient client, byte[] key, string usernameB, byte[] key_fingerprint, TextBox chatTextBox, TextBox messageTextBox)
        {
            this.key = key;
            this.stream = stream;
            this.client = client;
        ...
        public void SendMessage()
        {
            ...
            byte[] EncryptedData = subKey;

            //send_message
            byte[] message_bytes = new byte[key_fingerprint.Length + msg_key.Length + EncryptedData.Length];
            Buffer.BlockCopy(key_fingerprint, 0, message_bytes, 0, key_fingerprint.Length);
            Buffer.BlockCopy(msg_key, 0, message_bytes, key_fingerprint.Length, msg_key.Length);
            Buffer.BlockCopy(EncryptedData, 0, message_bytes, key_fingerprint.Length + msg_key.Length, EncryptedData.Length);

            //send_message to string
            //string send_message = System.Text.Encoding.Default.GetString(message_bytes); !!!!!!!!!!!!!!!!!!!!!
            string send_message = System.Text.Encoding.Default.GetString(EncryptedData);
            byte[] vs = Encoding.Default.GetBytes(send_message);

            //byte[] s1 = Encoding.Default.GetBytes("hi");
            //string s2 = System.Text.Encoding.Default.GetString(s1);
            //byte[] s3 = Encoding.Default.GetBytes(s2);
            ...
            //string s23 = System.Text.Encoding.Default.GetString(plaintext);
            //byte[] s33 = Encoding.Default.GetBytes(s23);
            //clear messageTextBox
            messageTextBox.Clear();
            string time = DateTime.Now.ToShortTimeString();
            chatTextBox.Text = chatTextBox.Text + time + " " + "Вы: " + message + "\r\n";
            ...
            while (true)
            {
                try
                {
                    ...
                    do
                    {
                        ...
                        //if (key1 != key2)
                        {
                            //MessageBox.Show("key_fingerprint != key_fingerprint_from_msg");
                        ...
                        {
                            //MessageBox.Show("msg_key != msg_key_from_msg");
                        }
                    ...
                    chatTextBox.Text = chatTextBox.Text + time + " " + usernameB + ": " + message + "\r\n";
                }
                catch (Exception exp)
                {
